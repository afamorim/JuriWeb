...
<html:html>
	<head>
		...
		<script language="JavaScript" type="text/JavaScript">
			function novo(){
				location.href = '<c:url value="/admin/Processo/Orgao/CarregarCadastro.do"/>';
			}
			check=false;
			function detalhar(codigo){
				if (!check){
					url = '<c:url value="/admin/Processo/Orgao/CarregarCadastro.do"/>';
					location.href =url+'?codigo='+codigo;
				}
				check=false;		
			}
			function excluir(){
				return excluirRegistros('codigosExclusao','<c:url value="/admin/Processo/Orgao/Remover.do"/>');
			}
		</script>
	
	</head>
	<body>
		<html:form action="/admin/Processo/Orgao/List" method="post">
			<div id="ConteudoSistema">    
				<h1><span><fmt:message key="lb_pesquisaOrgao"/></span></h1>
				<fieldset>
					<legend><fmt:message key="lb_pesquisaOrgao" /></legend>
					<p><html:errors/></p>
				...
				<table  border="0" cellpadding="2" cellspacing="1" width="715px">
					<tr class="td02">
						<td width="45%" class="th01"><fmt:message key="lb_orgao"/></td>
						<td width="45%" class="th01"><fmt:message key="lb_juizo"/></td>
					...
					<pg:pager url="${pageContext.request.requestURL}" maxPageItems="10" maxIndexPages="10" export="currentPageNumber=pageNumber"
						scope="request">
						<c:forEach items="${requestScope.orgaoJudiciarios}" var="item">
							<pg:item>
								<tr class="td02" onMouseOver="this.style.backgroundColor='#CCCCCC'" onMouseOut="this.style.backgroundColor='#eeeeee'" 
									...
									<td class="tdLeft">${item.juizo.descricao}</td>
									<td align="center" onclick="check=true;"><input type="checkbox" name="codigosExclusao" value="${item.codigo}"/></td>
								</tr>
							</pg:item>
						</c:forEach>  
						<tr>
							<td colspan="6" class="td02" align="center"><%@ include file='../navegador.jsp' %></td>
						</tr>
					</pg:pager>
				</table>
			</div>
