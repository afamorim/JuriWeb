...
public abstract class BaseAction extends Action {
	...
	public ActionForward tratarExcecao(Exception e,
        ...
        if (e instanceof AplicacaoException){
            ...
            try{
                ((BaseActionForm)form).carregarComponentes(pMapping, pRequest,this.getWebApplicationContext());
            }catch(Exception ex){
                ex.printStackTrace();
            }
                               ...
                               HttpServletResponse response) throws IOException, ServletException
  {
    try{
        ...
        LOG.debug("baseForm " + baseForm);
        if (baseForm != null)
        {
        	try{
                baseForm.carregarComponentes(mapping, request,this.getWebApplicationContext());
            }catch(Exception e){
                e.printStackTrace();
            }
  ...
  protected ActionForward exibirMensagem(HttpServletRequest request,
      ...
      saveErrors(request, erros);
      try{
          form.carregarComponentes(mapping, request,this.getWebApplicationContext());
      }catch(Exception ex){
          ex.printStackTrace();
      }
      return mapping.getInputForward();
