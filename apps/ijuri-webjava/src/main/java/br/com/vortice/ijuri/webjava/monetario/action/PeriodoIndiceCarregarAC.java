...
public class PeriodoIndiceCarregarAC extends BaseAction {
	...
	protected ActionForward executar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		PeriodoIndiceReajusteForm pirForm = (PeriodoIndiceReajusteForm)form;
		if (pirForm.getCodigo() != null && pirForm.getCodigo().intValue() > 0){
			PeriodoIndiceVO vo = new PeriodoIndiceVO();
			BeanUtils.copyProperties(vo, form);
			vo = correcaoMonetariaDelegate.findByPrimaryKey(vo);
			BeanUtils.copyProperties(pirForm, vo);
			pirForm.setIndiceReajusteCodigo(vo.getIndiceReajuste().getCodigo());
		}
