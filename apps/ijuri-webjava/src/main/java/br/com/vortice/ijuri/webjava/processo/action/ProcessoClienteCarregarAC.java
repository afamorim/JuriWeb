...
public class ProcessoClienteCarregarAC extends BaseAction {
			...
			throws Exception 
	{
		...
		if (usuario != null){
			if (request.getSession().getAttribute("pessoa") == null){
				PessoaVO pessoa = pessoaDelegate.findByUsuario(usuario);
		...
		Collection<ComarcaVO> comarcas = processoDelegate.findAllComarca();
