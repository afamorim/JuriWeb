...
public class LinkCarregarAC extends BaseAction {
	...
	protected ActionForward executar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (linkForm != null && linkForm.getCodigo() != null && linkForm.getCodigo().intValue() > 0){
			vo.setCodigo(linkForm.getCodigo());
			vo = siteDelegate.findByPrimaryKey(vo);
